...
function achievementNumericProgress(st){
 const hasCurrent=st?.currentValue!==null&&st?.currentValue!==undefined&&st?.currentValue!=="",hasTarget=st?.targetValue!==null&&st?.targetValue!==undefined&&st?.targetValue!=="";
 const current=hasCurrent&&Number.isFinite(Number(st.currentValue))?Math.max(0,Number(st.currentValue)):null,target=hasTarget&&Number.isFinite(Number(st.targetValue))?Math.max(0,Number(st.targetValue)):null;
 const remaining=current!==null&&target!==null?Math.max(target-current,0):null,percent=current!==null&&target!==null&&target>0?current/target*100:null;
 return {current,target,remaining,percent,numericComplete:current!==null&&target!==null&&target>0&&current>=target,unit:String(st?.progressUnit||"")};
}
function achievementNumericHtml(a,st){
 const x=achievementNumericProgress(st),id=esc(a.id),value=v=>v===null?"":esc(v);
 return `<div class="achievement-numeric-progress"><div class="achievement-numeric-fields"><label>現在数<input class="achCurrentValue" data-id="${id}" type="number" min="0" step="any" value="${value(x.current)}" placeholder="0"></label><label>目標数<input class="achTargetValue" data-id="${id}" type="number" min="0" step="any" value="${value(x.target)}" placeholder="未設定"></label><label>単位<input class="achProgressUnit" data-id="${id}" value="${esc(x.unit)}" placeholder="回・個・件など"></label></div><div class="achievement-numeric-summary" data-ach-progress-summary="${id}">${x.current!==null&&x.target!==null?`<b>${x.current.toLocaleString("ja-JP")} / ${x.target.toLocaleString("ja-JP")}${x.unit?` ${esc(x.unit)}`:""}</b><span>${x.numericComplete?"数値上 COMPLETE":`残り${x.remaining.toLocaleString("ja-JP")}${esc(x.unit)}`}</span>${x.percent!==null?`<span>${x.percent.toFixed(1)}%</span>`:""}`:'<span>現在数と目標数を入力すると、残数と達成率を表示します。</span>'}</div><div class="achievement-numeric-actions"><label><input class="achShowInGoals" data-id="${id}" type="checkbox" ${st.showInGoals===true?"checked":""}> GOALSに表示</label><button class="secondary achNumericSave" data-id="${id}" type="button">数値進捗を保存</button><span class="small" data-ach-progress-message="${id}" role="status"></span></div></div>`;
}
...
function achievementExtraHtml(a,st,tagCatalog=[]){
 ...
 <div class="achievement-checklist-box">
   ...
   <div class="achChecklist" data-id="${a.id}">
     ${(st.checklist||[]).length?(st.checklist||[]).map(i=>`<div class="listitem achievement-checklist-row"><div class="row"><label><input class="achSubCheck" data-ach="${a.id}" data-sub="${i.id}" type="checkbox" ${i.done?"checked":""}> ${esc(i.name)}${i.doneAt?` <span class="ach-check-date">${new Date(i.doneAt).toLocaleDateString("ja-JP")}</span>`:""}</label><button class="danger achSubDelete" data-ach="${a.id}" data-sub="${i.id}">削除</button></div></div>`).join(""):'<div class="small">項目はまだありません。</div>'}
   </div>
...
function renderAchievements(){
 ...
 document.querySelectorAll("[data-ach-kind]").forEach(x=>x.ontoggle=()=>{const kind=x.dataset.achKind;if(x.open&&!achievementOpenKinds.has(kind)){achievementOpenKinds.add(kind);renderAchievements()}else if(!x.open)achievementOpenKinds.delete(kind)});
 document.querySelectorAll("[data-ach-category]").forEach(x=>x.ontoggle=()=>{const key=x.dataset.achCategory;if(x.open&&!achievementOpenCategories.has(key)){achievementOpenCategories.add(key);renderAchievements()}else if(!x.open)achievementOpenCategories.delete(key)});
 ...
 document.querySelectorAll(".achNumericSave").forEach(b=>b.onclick=()=>saveAchievementNumericProgress(b.dataset.id));
 document.querySelectorAll(".achShowInGoals").forEach(c=>c.onchange=()=>saveAchievementNumericProgress(c.dataset.id,{renderAfter:true}));
...
function saveAchievementNumericProgress(id,{renderAfter=false}={}){
 const currentInput=document.querySelector(`.achCurrentValue[data-id="${CSS.escape(String(id))}"]`),targetInput=document.querySelector(`.achTargetValue[data-id="${CSS.escape(String(id))}"]`),unitInput=document.querySelector(`.achProgressUnit[data-id="${CSS.escape(String(id))}"]`),showInput=document.querySelector(`.achShowInGoals[data-id="${CSS.escape(String(id))}"]`),message=document.querySelector(`[data-ach-progress-message="${CSS.escape(String(id))}"]`);
 const parseOptional=input=>{const raw=String(input?.value??"").trim();if(raw==="")return null;const n=Number(raw);return Number.isFinite(n)&&n>=0?n:NaN};
 const current=parseOptional(currentInput),target=parseOptional(targetInput);if(Number.isNaN(current)||Number.isNaN(target)){if(message)message.textContent="0以上の数値を入力してください。";return false}
 const d=load();d.achievementProgress=d.achievementProgress||{};const st=d.achievementProgress[id]||{done:false,date:"",note:"",checklist:[],tags:[]};
 st.currentValue=current;st.targetValue=target;st.progressUnit=String(unitInput?.value||"").trim();st.showInGoals=showInput?.checked===true;d.achievementProgress[id]=st;save(d);
 if(renderAfter){renderAchievements();renderGoals()}else{if(message)message.textContent="保存しました。";renderGoals()}
 return true;
}
...
let craftPage=1;
const craftOpenJobs=new Set(),craftOpenBands=new Set(),craftOpenProducts=new Set(),craftOpenLeves=new Set(),craftOpenMaterials=new Set(),craftOpenLeveGroups=new Set();
...
function craftRecipeMeta(recipe){
 const row=(typeof CRAFT_RECIPE_META_DATA!=="undefined"&&CRAFT_RECIPE_META_DATA[recipe.id])||[0,0];
 return {notebookId:Number(row[0])||0,statusRequiredId:Number(row[1])||0};
}
function craftItemMeta(itemId){
...
function craftDisplayTags(recipe,linked){
 ...
 if(recipe.specialist)tags.push('<span class="badge craft-specialist">マイスター</span>');
 if(meta.statusRequiredId)tags.push('<span class="badge craft-facility">製作施設条件</span>');
 if(linked.length)tags.push(`<span class="badge green">リーヴ${linked.length>1?`×${linked.length}`:""}</span>`);
 return tags.join("");
}
...
function filteredCraftRecipes(index=craftLeveIndex()){
 ...
 return CRAFT_RECIPE_DATA.filter(r=>{
  ...
  const classificationOk=!classification
   ...
   ||(classification==="secret"&&isSecret)
   ||(classification==="cosmo"&&isCosmo);
  const relatedOk=!related
...
function renderCrafting(){
 ...
 document.querySelectorAll("[data-craft-job]").forEach(x=>x.ontoggle=()=>{if(filterActive)return;const k=x.dataset.craftJob;if(x.open&&!craftOpenJobs.has(k)){craftOpenJobs.add(k);renderCrafting()}else if(!x.open)craftOpenJobs.delete(k)});
 document.querySelectorAll("[data-craft-band]").forEach(x=>x.ontoggle=()=>{if(filterActive)return;const k=x.dataset.craftBand;if(x.open&&!craftOpenBands.has(k)){craftOpenBands.add(k);renderCrafting()}else if(!x.open)craftOpenBands.delete(k)});
 ...
 document.querySelectorAll("[data-craft-tree-materials]").forEach(x=>x.ontoggle=async()=>{const k=String(x.dataset.craftTreeMaterials);if(!x.open){craftOpenMaterials.delete(k);return}craftOpenMaterials.add(k);const box=x.querySelector(".craft-tree-material-body");if(!box||box.dataset.loaded)return;box.innerHTML='<div class="small">素材を読み込んでいます…</div>';try{const rows=await fetchCraftMaterials(Number(k));box.innerHTML=renderMaterialRows(rows);box.dataset.loaded="1";bindCraftSubButtons(box)}catch(e){console.error(e);box.innerHTML='<div class="empty">素材データを取得できませんでした。</div>'}});
...
function renderGoals(){
 ...
 const manual=(d.goals||[]).map(g=>({kind:"manual",key:`manual:${g.id}`,id:String(g.id),goal:g,x:goalDerived(g),priority:g.priority||3,deadline:g.deadline||"",createdAt:g.createdAt||0,done:goalDerived(g).complete}));
 const achievementMaster=new Map((typeof ACHIEVEMENT_DB!=="undefined"?ACHIEVEMENT_DB:[]).map(a=>[String(a.id),a]));
 const achievement=Object.entries(d.achievementProgress||{}).filter(([id,st])=>st?.showInGoals===true&&achievementMaster.has(String(id))).map(([id,st])=>{const a=achievementMaster.get(String(id)),x=achievementNumericProgress(st);return {kind:"achievement",key:`achievement:${id}`,id:String(id),achievement:a,state:st,x,priority:0,deadline:"",createdAt:0,done:st.done===true}});
 let goals=[...manual,...achievement].filter(g=>showDone||!g.done);
 const remaining=g=>g.kind==="manual"?g.x.remaining:(g.x.remaining??Number.POSITIVE_INFINITY),rate=g=>g.kind==="manual"?g.x.rate:(g.x.percent??-1);
 const cmp={priority:(a,b)=>b.priority-a.priority||a.kind.localeCompare(b.kind)||a.createdAt-b.createdAt,deadline:(a,b)=>(a.deadline||"9999-99-99").localeCompare(b.deadline||"9999-99-99")||a.kind.localeCompare(b.kind),remaining:(a,b)=>remaining(a)-remaining(b),rate:(a,b)=>rate(b)-rate(a),created:(a,b)=>a.createdAt-b.createdAt||a.kind.localeCompare(b.kind)}[sort];goals.sort(cmp);
 list.innerHTML=goals.length?goals.map(item=>{const open=goalOpenIds.has(item.key);if(item.kind==="manual"){const g=item.goal,x=item.x,deadline=goalDeadlineText(g.deadline),unit=esc(g.unit||"");return `<details class="goal-item ${x.complete?"is-done":""}" data-goal-key="${esc(item.key)}" ${open?"open":""}><summary><b>${esc(g.name||"名称未設定")}</b><span class="goal-priority">${esc(GOAL_PRIORITY_LABELS[g.priority]||GOAL_PRIORITY_LABELS[3])}</span><span>${x.current.toLocaleString("ja-JP")} / ${x.target.toLocaleString("ja-JP")} ${unit}</span><span>${x.complete?"COMPLETE":`あと${x.remaining.toLocaleString("ja-JP")}${unit}`}</span><span>${x.rate.toFixed(1)}%</span></summary>${open?`<div class="goal-item-body"><div class="goal-meter"><i style="width:${Math.min(100,Math.max(0,x.rate))}%"></i></div><div class="goal-facts">${g.startDate?`<span>開始 ${esc(g.startDate)}</span>`:""}${g.deadline?`<span>期限 ${esc(g.deadline)}</span>`:""}${deadline?`<span class="${deadline.startsWith("期限超過")?"goal-overdue":""}">${esc(deadline)}</span>`:""}</div>${g.note?`<p>${nl(g.note)}</p>`:""}<div class="wrap"><button class="secondary goalEdit" data-id="${esc(g.id)}" type="button">編集</button><button class="danger goalDelete" data-id="${esc(g.id)}" type="button">削除</button></div></div>`:""}</details>`}const a=item.achievement,st=item.state,x=item.x,unit=esc(x.unit);return `<details class="goal-item goal-achievement ${st.done?"is-done":""}" data-goal-key="${esc(item.key)}" ${open?"open":""}><summary><b>${esc(a.name)}</b><span class="badge">アチーブメント</span><span>${x.current===null?"—":x.current.toLocaleString("ja-JP")} / ${x.target===null?"—":x.target.toLocaleString("ja-JP")} ${unit}</span><span>${st.done?"正式達成済み":x.numericComplete?"数値上 COMPLETE":x.remaining!==null?`あと${x.remaining.toLocaleString("ja-JP")}${unit}`:"目標未設定"}</span>${x.percent!==null?`<span>${x.percent.toFixed(1)}%</span>`:""}</summary>${open?`<div class="goal-item-body"><div class="goal-meter"><i style="width:${Math.min(100,Math.max(0,x.percent||0))}%"></i></div><div class="goal-achievement-edit"><label>現在数<input class="goalAchievementCurrent" data-achievement-id="${esc(item.id)}" type="number" min="0" step="any" value="${x.current===null?"":esc(x.current)}"></label><button class="secondary goalAchievementSave" data-achievement-id="${esc(item.id)}" type="button">進捗を保存</button><span class="small" data-goal-achievement-message="${esc(item.id)}" role="status"></span></div><div class="wrap"><button class="secondary goalAchievementOpen" data-achievement-id="${esc(item.id)}" type="button">元のアチーブメントを開く</button></div></div>`:""}</details>`}).join(""):'<div class="empty">表示するGOALはありません。</div>';
 document.querySelectorAll("[data-goal-key]").forEach(el=>el.ontoggle=()=>{const key=String(el.dataset.goalKey);if(el.open&&!goalOpenIds.has(key)){goalOpenIds.add(key);renderGoals()}else if(!el.open)goalOpenIds.delete(key)});
 ...
 document.querySelectorAll(".goalDelete").forEach(b=>b.onclick=e=>{e.stopPropagation();if(!confirm("このGOALを削除しますか？"))return;const data=load();data.goals=(data.goals||[]).filter(g=>String(g.id)!==String(b.dataset.id));goalOpenIds.delete(`manual:${String(b.dataset.id)}`);if(goalEditId===String(b.dataset.id))clearGoalForm();save(data);renderGoals();renderGoalCalendar()});
 document.querySelectorAll(".goalAchievementSave").forEach(b=>b.onclick=e=>{e.stopPropagation();const id=String(b.dataset.achievementId),input=document.querySelector(`.goalAchievementCurrent[data-achievement-id="${CSS.escape(id)}"]`),message=document.querySelector(`[data-goal-achievement-message="${CSS.escape(id)}"]`),raw=String(input?.value??"").trim(),value=raw===""?null:Number(raw);if(value!==null&&(!Number.isFinite(value)||value<0)){if(message)message.textContent="0以上の数値を入力してください。";return}const data=load(),st=data.achievementProgress?.[id];if(!st)return;if(value===null)st.currentValue=null;else st.currentValue=value;save(data);renderGoals()});
 document.querySelectorAll(".goalAchievementOpen").forEach(b=>b.onclick=e=>{e.stopPropagation();openAchievementFromGoal(b.dataset.achievementId)});
}
function openAchievementFromGoal(id){
 const a=(typeof ACHIEVEMENT_DB!=="undefined"?ACHIEVEMENT_DB:[]).find(x=>String(x.id)===String(id));if(!a)return;
 switchView("achievements");if($("achSearch"))$("achSearch").value=a.name;if($("achKind"))$("achKind").value="";if($("achCategory"))$("achCategory").value="";if($("achStatus"))$("achStatus").value="";achievementOpenKinds.add(a.kind);achievementOpenCategories.add(achievementCategoryKey(a.kind,a.category));renderAchievements();requestAnimationFrame(()=>document.querySelector(`[data-ach-item="${CSS.escape(String(id))}"]`)?.scrollIntoView({behavior:"smooth",block:"center"}));
}
function saveGoalFromForm(){
 const message=$("goalMessage");if(message)message.textContent="";
 try{
  const name=$("goalName")?.value.trim(),current=goalNumber($("goalCurrent")?.value),target=goalNumber($("goalTarget")?.value);if(!name){if(message)message.textContent="目標名を入力してください。";return alert("目標名を入力してください。")}if(target<0||current<0){if(message)message.textContent="現在値と目標値は0以上で入力してください。";return alert("現在値と目標値は0以上で入力してください。")}
  ...
  d.goals=Array.isArray(d.goals)?d.goals:[];if(old)Object.assign(old,goal);else d.goals.push(goal);goalOpenIds.add(`manual:${String(goal.id)}`);save(d);
  if(!(LIVE_DATA.goals||[]).some(g=>String(g.id)===String(goal.id)))throw new Error("GOALを保存状態へ反映できませんでした");
  clearGoalForm();renderGoals();renderGoalCalendar();if(message)message.textContent=`「${goal.name}」を保存しました。`;
 }catch(error){console.error("GOAL save failed",error);if(message)message.textContent="GOALを登録できませんでした。入力内容は残しています。画面上部の保存状態とコンソールを確認してください。";setSaveStatus("⚠ GOALを保存できませんでした",false)}
}
function renderGoalCalendar(){
 ...
 document.querySelectorAll("[data-calendar-goal]").forEach(b=>b.onclick=()=>{const id=String(b.dataset.calendarGoal),key=`manual:${id}`;goalOpenIds.add(key);switchView("goals");requestAnimationFrame(()=>document.querySelector(`[data-goal-key="${CSS.escape(key)}"]`)?.scrollIntoView({behavior:"smooth",block:"center"}))});
}
